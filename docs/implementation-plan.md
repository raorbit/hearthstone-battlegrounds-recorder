...
| MMR | Optional `IRatingProvider`. **Decided (M1 exit, 2026-07-14): HearthMirror is licensing NO-GO** (see `spikes/SpikeC.MmrRoute/LICENSING.md`), and the HearthSim-permission route was declined (2026-07-16). The clean-room external reader (LICENSING path b) is now **built** in `BgRecorder.Rating` behind `EnableMemoryRating` — default OFF until its Mono offsets are verified live (`docs/mmr-offset-verification.md`); the null provider ships until then. Recording never depends on it |
