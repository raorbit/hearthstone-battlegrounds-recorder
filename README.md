...
Still needed to close M2: wire the onboarding live test-feed verifier, run 3 consecutive hands-free real matches including a client restart, check A/V sync within ±100 ms over a long session, and complete Spike B's 30-minute PresentMon measurement. M3 still needs real multi-GB seek/scrub validation and marker timing checks across real matches; thumbnails, keyboard shortcuts, and virtual clips are in, and final-board/damage presentation is deferred until live matches supply that data. Automatic MMR has a clean-room external memory reader, shipped **off by default**: the 2026-07-16 live probe verified the Mono ABI offsets but found the documented field path gone — no `BaconRatingMgr` exists in the current build, and the rating sits behind a `NetCache` singleton with no known static root — so the feature waits on a field-path redesign (see [`docs/mmr-offset-verification.md`](docs/mmr-offset-verification.md)); manual rating entry remains the default source.
...
- Rating (MMR) tracking per mode (solo/duos): manual entry always available; automatic memory-read tracking via a clean-room external reader, shipped off by default until its NetCache field-path redesign lands — the Mono ABI offsets are already live-verified (see `docs/mmr-offset-verification.md` and `spikes/SpikeC.MmrRoute/LICENSING.md`)
