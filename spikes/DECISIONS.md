...
- **DECIDED 2026-07-14: v1 ships without MMR (path c); the clean-room reader (path b) is the planned post-v1 route.** `IRatingProvider` stays in the architecture with a null implementation so the reader can slot in later without rework; M4 shrinks to the degradation UX + manual-entry stub. The HearthSim email remains worth sending — a permission grant would upgrade the post-v1 route from clean-room reader to HearthMirror proper.
