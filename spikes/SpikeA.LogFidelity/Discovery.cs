...
///
/// Modern clients (patch ~25.0.4+) write per-session timestamped subfolders. On this machine only the
/// newest session folder holds a live "Power.log"; older folders keep a rotated "Power_old.log" and have
/// no "Power.log" at all. We pick the newest folder by its parsed name timestamp (falling back to
/// directory creation time when the name does not parse), then take that folder's Power.log. If the very
/// newest folder happens to lack a Power.log we fall through to the newest folder that does have one, so
/// the command never returns a non-existent path when a usable log exists.
/// </summary>
public static partial class Discovery
{
    public const string DefaultInstallDir = @"C:\Program Files (x86)\Hearthstone";
    ...
    public static Result Discover(string installDir)
    {
        ...
        // Chosen = newest folder overall; but if it has no Power.log, fall through to the
        // newest folder that does (still reporting the overall-newest as "chosen" context).
